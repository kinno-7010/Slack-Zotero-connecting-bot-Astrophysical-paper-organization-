"""Slack Paper Inbox Bot: direct Zotero registration edition.

Slackの専用チャンネルに投稿された論文リンクを処理し、Slack Listsを介さず
Zoteroの指定コレクションへ直接登録します。Slack Freeプランでも利用できます。

利用者固有のトークン、ID、メールアドレス、フォルダーの絶対パスはコード内に
...
def handle_message_events(
    ...
    LOGGER.info(
        "Received Slack message: channel=%s ts=%s text=%r",
        channel,
        message_ts,
        text,
    )
    ...
    try:
        ...
        if not paper.get("title"):
            say(
                text=(
                    "⚠️ DOIは取得できましたが、タイトルを取得できなかったため"
                    "登録を停止しました。\n"
                    f"DOI: {paper['doi_url']}"
                ),
                thread_ts=message_ts,
        ...
        say(
            text=(
                ...
                f"*Title:* {paper['title']}\n"
                f"*DOI:* {paper['doi_url']}\n"
                f"*Zotero:* {describe_zotero_status(zotero_result)}\n"
