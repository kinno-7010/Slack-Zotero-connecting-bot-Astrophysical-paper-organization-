...
Slackの専用チャンネルに論文リンクを投稿すると、論文情報を自動取得し、次の2か所へ登録するPython Botです。

1. Slackの「読みたい論文リスト」
2. Zoteroの指定コレクション

また、すでにSlackリストに蓄積されている論文をZoteroへ一括同期するスクリプトも利用できます。

ただし、Slackが無料プランだとリスト機能が使えませんが、その場合でも、Zoteroに直接登録することはできます。
---
...
通常運用では、**`paper_inbox_bot_zotero.py` を実行してください**。
...
この値を後で `.env` の `SLACK_APP_TOKEN` に保存します。

## 3.3 Bot Token Scopesを設定する
...
| `lists:read` | Slackリストの行・列を読む |
| `lists:write` | Slackリストへ行を追加 |
...
## 5.2 Zotero User IDを取得する

API Keyを使って次へアクセスすると、数値のUser IDを取得できます。
...
## 5.3 Collection Keyを取得する

Zotero APIでコレクション一覧を取得します。

```text
GET https://api.zotero.org/users/<USER_ID>/collections
```
...
Botに設定するのは、最下層の `未印刷` のcollection keyです。
...
SLACK_COL_CITATION=Col0123456789
SLACK_COL_TITLE=Col0123456789
SLACK_COL_DOI_LINK=Col0123456789
SLACK_COL_MEMO=Col0123456789
SLACK_COL_MESSAGE=Col0123456789
...
`ExecStart` は必ず **`paper_inbox_bot_zotero.py`** を指定してください。
...
| Slack List | `.env` の `SLACK_LIST_ID` |
| Slack列構成 | 5つの `SLACK_COL_*` |
...
- Slack Lists API: <https://docs.slack.dev/reference/methods/slackLists.items.create>
